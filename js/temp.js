...
var MainManager = function() {
	...
	function A() {
		...
		W ? ($("#scene_loading").stop().css("display", "none"), $("#bt_back").stop().css("display", "none"), ea && $("#bt_fs").stop().css("display", "none"), $("#scene_main .set").stop().css("display", "none")) : $("#scene_main").stop().fadeIn(200, "linear");
		Y = aidn.___waContext.currentTime;
		P.start();
		Q.start()
	}
	function K(a) { (ia = !ia) ? ($("#bt_feedback a").text("FEEDBACK: ON"), aidn.util.setCookie("fb", "on", 2592E3)) : ($("#bt_feedback a").text("FEEDBACK: OFF"), aidn.util.setCookie("fb", "off", 2592E3));
		a && a.preventDefault()
	}
	function L(a) { (Z = !Z) ? ($("#bt_backtrack a").text("BACKTRACK: ON"), aidn.util.setCookie("bt", "on", 2592E3)) : ($("#bt_backtrack a").text("BACKTRACK: OFF"), aidn.util.setCookie("bt", "off", 2592E3));
		a && a.preventDefault()
	}
	function M() {
		Q.update();
		...
		if (W && 1 == S) {
			...
			if (F * u < a) {
				...
				p += e - F;
				F = e;
				if (0 <= F * u - a) for (e = Math.random(), a = 1, 192 <= p ? p = 0 : 128 <= p ? (.7 > e && (a = 2), .5 > e && (a = 3)) : 64 <= p ? (.35 > e && (a = 2), .2 > e && (a = 3), .02 > e && (a = 0)) : 32 <= p ? (.35 > e && (a = 2), .24 > e && (a = 0)) : 0 <= p && .4 > e && (a = 0), e = 0; e < a; e++) fa = X[V],
				V++,
				32 <= V && (V = 0, aidn.util.shuffleArray(X)),
				P.changeId(fa, 0, !0)
			}
		}
		O.render(ga);
	...
	for (var F = 0,
	u = 6E4 / 280,
	fa = Math.floor(32 * Math.random()), p = 0, X = [], V = 0, x = 0; 32 > x; x++) X[x] = x;
	var W = !1;
	...
	$("#bt_start a").click(function(a) {
		...
		2 == R ? A() : ((new aidn.WebAudio).load(""), Q.init(z, J));
		a.preventDefault()
	});
	...
	function() {
		...
		this.update = function() {
			if (p) {
				var a = 1E3 * (aidn.___waContext.currentTime - Y);
				if (U * la < a && (U = Math.floor(a / la) + 1, a = U * la - a, 0 <= a && Z)) for (var e = (U - 1) % x, r = v.length, t = 0; t < r; t++) {
					var G = v[t][e];
	...
	function() {
		function p(a, g) {
			...
			f = 0; f < e; f++) {
				var b = I[f];
				if (b.hitcheck(a, g)) return G != b.id && b.play(),
				b.id
			}
			return ! 1
		}
		...
		function x(a) {
			...
			e = p(g.x, g.y);
			r(e);
			if (a.originalEvent && a.originalEvent.touches) for (var g = a.originalEvent.touches.length,
			f = 1; f < g; f++) e = a.originalEvent.touches[f],
			e = p(e.pageX, e.pageY),
			r(e, 1)
		}
		function z(a) {
		...
		fa = function(m) {
			...
			this.flash = function() {
				...
				for (var a = 0; 3 > a; a += 2) TweenLite.delayedCall(.07 * a, g),
				TweenLite.delayedCall(.07 * (a + 1), q)
			};
		...
		O = function(a, e) {
			function q() {
				...
				n.moveTo(0, 0);
				var e, b;
				if (0 == d) for (var k = 0; k < c.rotation; k += 30) e = (l * k + B) * Math.PI / 180,
				b = Math.cos(e) * a,
				e = Math.sin(e) * a,
				n.lineTo(b, e);
				else for (k = 360; c.rotation < k; k -= 30) e = (l * k + B) * Math.PI / 180,
				b = Math.cos(e) * a,
				e = Math.sin(e) * a,
				n.lineTo(b, e);
		...
		this.start = function() {
			...
			if (ba || window.TouchEvent) $("#view").on("touchstart", x),
			$(window).on("touchmove", z),
			$(window).on("touchend", E);
			$("#view").css("cursor", "pointer")
		};
		this.end = function() {
			ba || ($("#view").off("mousedown", x), $(window).off("mousemove", z), $(window).off("mouseup", E), $(window).off("keydown", u), $(window).off("keyup", A));
			if (ba || window.TouchEvent) $("#view").off("touchstart", x),
			$(window).off("touchmove", z),
			$(window).off("touchend", E);
		...
		function(m, g) {
			...
			this.play = function() {
				...
				for (var b = 10,
				l = v(), y = Math.min(a, e) / 64 * (.6 * Math.random() + .7), n = 2, w = 0; 40 > w; w++) {
		...
		function(m, g) {
			...
			var f = function(c) {
				function b() {
					var c = Math.min(a, e) * (.05 * Math.random() + .014);
					k.beginFill(v());
					k.drawCircle(0, 0, c);
					k.x = f;
					k.y = g;
		...
		function(m, g) {
			...
			this.play = function() {
				...
				w = v(),
				m = Math.PI / 2 * Math.floor(4 * Math.random()),
				r = 1;.5 > Math.random() && (r = -1);
				for (var k = 0; k < b; k++) {
		...
		function(m, g) {
			...
			this.play = function() {
				...
				c.scale.x = 0;
				h.scale.y = 0;
				f = .5 > Math.random() ? -135 * Math.PI / 180 : 215 * Math.PI / 180; (new TimelineLite).to(c.scale, .4, {
					x: 1,
					ease: Power2.easeOut
				}).to(h.scale, .4, {
					y: 1,
		...
		function(m, g) {
			...
			this.play = function() {
				n.clear();
				n.visible = !0;.5 > Math.random() ? (n.x = 0, n.y = 0, n.rotation = 0) : (n.x = a, n.y = e, n.rotation = Math.PI);
				d = h = 0;
		...
		R = [function(m, g) {
			...
			this.play = function() {
				...
				var g = t();
				c = W[g];
				$("#about").css("background-color", "#" + c.toString(16));.3 > Math.random() && ca.flash(p);
				Z = g;
				l.clear();
				l.visible = !0;
		...
		Y = [13430510, 8965324, 9099756, 961181, 1089457, 34969, 13934238, 16110792, 15488645, 16531063, 5853015, 3222317],
		aa = W.length,
		Z = 0,
		ca,
		oa = 0
	...
	ha = !1,
	na = 0,
	ia = "off" == aidn.util.getCookie("fb"),
	Z = "off" == aidn.util.getCookie("bt");
	K();
	L();
	aidn.util.webaudio ? ($("#ng").css("display", "none"), $(".ok").css("display", "block"), ba && $("#scene_main .attention").html("TOUCH &amp; SWIPE!"), x || $("#scene_top .attention").text("* Raise the volume and enjoy!")) : ($("#ng").css("display", "block"), $(".ok").css("display", "none"), x || $("#ng .atten").html("Sorry,<br>your device or browser doesn't support this site."));
